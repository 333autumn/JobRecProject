...
class CommentJobs(models.Model):  # 评论表
...
class Jobs(models.Model):  # 职位表
    id = models.BigAutoField(primary_key=True)  # 主键自增
    job_id = models.IntegerField(blank=True, null=True)  # 数字类型
    jobid = models.CharField(max_length=255, blank=True, null=True)  # 字符类型
    ...
    jobsummary = models.TextField(blank=True, null=True)  # 文本类型
    ...
    publishtime = models.DateTimeField(blank=True, null=True)  # 日期类型
    ...
    class Meta:
        # managed = False
        verbose_name = '职位列表'  # 管理界面中显示的名称
        db_table = 'jobs'  # 数据库表名称


class UserResume(models.Model):  # 求职者简历表
    ...
    username = models.CharField(max_length=255, verbose_name='用户姓名', null=True, blank=True)
    # email
    # phone
...
class company(models.Model):  # 公司表
...
class StarJobs(models.Model):  # 收藏工作表
...
class ClickJobs(models.Model):  # 点击工作表
    cid = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('api.user', verbose_name="用户", on_delete=models.CASCADE)
    job = models.ForeignKey('api.Jobs', verbose_name="岗位", on_delete=models.CASCADE)
...
class ClickTalents(models.Model):  # 点击人才表
    cid = models.BigAutoField(primary_key=True)
    # 公司
    company = models.ForeignKey('api.user', verbose_name="用户", on_delete=models.CASCADE, related_name='user_id')
    # 人才
    user = models.ForeignKey('api.user', verbose_name="用户", on_delete=models.CASCADE, related_name='click_id')
    count = models.IntegerField(default=1)
    create_time = models.DateTimeField(auto_now=False, auto_now_add=True, verbose_name='创建时间')
    last_update = models.DateTimeField(auto_now=True, auto_now_add=False, verbose_name='最后修改时间')
