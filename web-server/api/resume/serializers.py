...
from api.auth.serializers import UserInfoSerializer
from api.models import UserResume, user


class UserResumeSerializer(serializers.ModelSerializer):  # 用户简历序列化器
...
class UserRecommendSerializer(serializers.ModelSerializer):  # 推荐用户序列化器
    user = UserInfoSerializer()

    class Meta:
        model = UserResume
        fields = ['eduHighestLevelTranslation', 'workingexp','worktype','skilllabel','preferredSalaryMin', 'user']
...
class baseinfoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = user
        fields = ['name', 'birth', 'genderCode', 'genderTranslation', 'currentIdentity', 'currentCity',
                  'currentCityTranslation', 'currentCityDistrictId', 'currentCityDistrictIdTranslation',
                  'currentProvince', 'currentProvinceTranslation', 'politicalAffiliation']
