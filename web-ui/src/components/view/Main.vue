<template>
	...
	<div class="hotjob" style="margin-top: 50px;margin-bottom: 40px;">
		<h1>热招岗位</h1>
		<el-menu :default-active="jobtypeinit" class="el-menu-hotjob" mode="horizontal" :ellipsis="true"
			@select="getHotJob" style="height: 45px">
			<div class="flex-grow" />
        ...
        job.workingexp }}</span><span v-if="job.education">&nbsp;|&nbsp;{{ job.education }}</span>
						...
						<div class="tags">
							...
							<template v-for="tag2 in job.welfaretaglist.split('/').slice(0, 2)"
								v-if="job.welfaretaglist">
			...
			<template v-for="company in HotCompany.slice(0, 9)">
				<div class="hotcompany-item" v-show="hotcompanystatus">
					<div class="card-header">
						<el-badge value="Hot" class="item">
							<div class="card-title">
								<el-tooltip class="box-item" effect="dark" :content="company.companyname"
									placement="bottom">
									<a
										:href="'/company/detail/'+company.companynumber">{{ company.companyname.slice(0, 10) }}...</a>
								</el-tooltip>
							</div>
						</el-badge>
	...
	export default {
		...
		data() {
			return {
				...
				imgs: [{
						url: img1,
						href: 'https://edu.51testing.net/htm/51PC/zlzs.html'
					},
					{
						url: img2,
						href: 'http://zhaopin.chinatowercom.cn/campus'
					},
					{
						url: img3,
						href: 'https://xiaoyuan.zhaopin.com/company/KA0133889514D90000001000'
					}
				],
				HotJob: [],
				hotjobstatus: false,
